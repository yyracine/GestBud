...
| **3.4** | **Validation globale + regroupement Historique** | **⏳ à faire** |
...
## Story 3.4 — prochaine à implémenter

**Titre :** Validation globale du Reçu et regroupement dans l'Historique

**ACs clés :**
- AC-1 : `TransactionRepository.insertReceiptLines(receiptId, lines)` — insertion atomique `database.transaction()`
- AC-2 : Navigation vers Accueil après validation + snack « Reçu enregistré ! »
- AC-3 : Entrée groupée dans l'Historique « Reçu · N articles · total FCFA » avec chevron
- AC-4 : Expand/collapse des lignes individuelles (UX-DR19), `AnimatedSize`, Reduce Motion
- AC-5 : Annonce VoiceOver/TalkBack sur l'entrée groupée
- AC-6 : Tap sur ligne individuelle développée → écran Détail/Modification (Story 2.4)

**Nouveaux fichiers attendus :**
- `lib/shared/widgets/receipt_group_tile.dart` — entrée groupée expand/collapse
- Mise à jour `lib/shared/data/transaction_repository.dart` — méthode `insertReceiptLines`
- Mise à jour `lib/features/scan/screens/scan_review_screen.dart` — activer `_ValidateButton`
- Mise à jour `lib/features/transactions/screens/history_screen.dart` — afficher `ReceiptGroupTile`
- Tests unitaires `test/shared/data/transaction_repository_test.dart` — `insertReceiptLines`

**Notes :**
- `receipt_id TEXT` nullable est déjà dans le schéma Drift (défini en Story 1.1)
- `_ValidateButton` a `onPressed: null` → doit appeler `_validateReceipt()` (Story 3.4)
- UUID v4 pour `receiptId` généré dans `insertReceiptLines` (pas dans le widget)
- Reduce Motion : `MediaQuery.disableAnimations` pour `AnimatedSize`
...
| `lib/features/scan/screens/scan_review_screen.dart` | `ConsumerStatefulWidget`, sticky header, état vide |
...
**Dernier run :** 62/62 tests passent · `flutter analyze` : 0 issues
